...
export default defineComponent({
	...
	sockets: {
		...
		receivedFTFavoriteMessage: function (data) {
		...
		receivedFT_HDP_OU_FAVORITE: function (data) {
			this.loading = false;
		...
		receivedFT_CORNER_Data: function (data) {
			this.loading = false;
	...
	methods: {
		...
		ftDataChange: function (ftDataList) {
			...
			ftDataList.forEach(ftData => {
				...
				ftData.forEach(item => {
					...
					let handicap_sign_m = "";
					let half_handicap_sign_m = "";
					let handicap_sign_t = "";
					let half_handicap_sign_t = "";

					if (item["showType"] == "rb") {
						handicap_sign_m = item["ShowTypeRB"] == "H" ? "-" : "+";
						half_handicap_sign_m = item["ShowTypeHRB"] == "H" ? "-" : "+";
						handicap_sign_t = item["ShowTypeRB"] == "H" ? "+" : "-";
						half_handicap_sign_t = item["ShowTypeHRB"] == "H" ? "+" : "-";
								...
								data: [
									{
										...
										nums: [
											...
											},
											{
												...
												colorChangeUp: false,
												colorChangeDOwn: false,
												text: item["M_LetB_RB_1"] !== "" ? handicap_sign_m + item["M_LetB_RB_1"] : "",
						...
						half_handicap_sign_m = item["ShowTypeHR"] == "H" ? "-" : "+";
						handicap_sign_t = item["ShowTypeR"] == "H" ? "+" : "-";
						half_handicap_sign_t = item["ShowTypeHR"] == "H" ? "+" : "-";
						if (item["HDP_OU"] === 1) {
									...
									nums: [
										{
											...
											colorChangeUp: false,
											colorChangeDOwn: false,
											text: (Number(item["MB_LetB_Rate_RB"])) > (Number(item["TG_LetB_Rate_RB"])) ? handicap_sign_m + item["M_LetB_RB"] : "+" + item["M_LetB_RB"],
											num: item["MB_LetB_Rate_RB"] == 0 ? 0 : (Number(item["MB_LetB_Rate_RB"])).toFixed(2)
										},
										{
											lineType: 10,
									...
									nums: [
										{
											...
											colorChangeUp: false,
											colorChangeDOwn: false,
											text: (Number(item["MB_LetB_Rate_RB"])) < (Number(item["TG_LetB_Rate_RB"])) ? handicap_sign_t + item["M_LetB_RB"] : "+" + item["M_LetB_RB"],
											num: item["TG_LetB_Rate_RB"] == 0 ? 0 : (Number(item["TG_LetB_Rate_RB"])).toFixed(2)
										},
										{
											lineType: 10,
									...
									nums: [
										{
											...
											colorChangeUp: false,
											colorChangeDOwn: false,
											text: (Number(item["MB_LetB_Rate"])) > (Number(item["TG_LetB_Rate"])) ? handicap_sign_m + item["M_LetB"] : "+" + item["M_LetB"],
		...
		handleModal: function (leagueData, gameData, dataList, rateData, scoreIndex) {
			...
			if (gameData["showType"] === "rb") {
				...
				if (this.bettingOrderData["title"].includes("让球") && this.user.FT_RE_Bet == 0) {
					showToast("对不起,本场有下注金额最高:  RMB 0");
					return;
				}
				if (this.bettingOrderData["title"].includes("大小") && this.user.FT_ROU_Bet == 0) {
					showToast("对不起,本场有下注金额最高:  RMB 0");
					return;
				}
		...
		handleSummaryModal1: function (leagueData, gameData, title, dataList, rateData) {
			...
			if (gameData["showType"] === "rb") {
				...
				if (title.includes("让球") && this.user.FT_RE_Bet == 0) {
					showToast("对不起,本场有下注金额最高:  RMB 0");
					return;
				}
				if (title.includes("大小") && this.user.FT_ROU_Bet == 0) {
					showToast("对不起,本场有下注金额最高:  RMB 0");
					return;
				}
				if (title.includes("独赢") && this.user.FT_RM_Bet == 0) {
					showToast("对不起,本场有下注金额最高:  RMB 0");
					return;
				}
				...
				if (title.includes("让球") && this.user.FT_R_Bet == 0) {
					showToast("对不起,本场有下注金额最高:  RMB 0");
					return;
				}
				if (title.includes("大小") && this.user.FT_OU_Bet == 0) {
					showToast("对不起,本场有下注金额最高:  RMB 0");
					return;
				}
				if (title.includes("独赢") && this.user.FT_M_Bet == 0) {
					showToast("对不起,本场有下注金额最高:  RMB 0");
					return;
				}
