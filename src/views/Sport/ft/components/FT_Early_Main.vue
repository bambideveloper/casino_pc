...
export default defineComponent({
	...
	sockets: {
		...
		receivedFTEarlyMessage: function (data) {
			this.loading = false;
		...
		receivedFT_HDP_OU_EARLY: function (data) {
	...
	mounted() {
		console.log("ok");
		console.log(this.earlyGids);
		this.$socket.emit("sendFTEarlyMessage", { lids: this.earlyGids, field: this.field });
	},
	unmounted() {
		this.$socket.emit("stopFTEarlyMessage")
	},
	methods: {
		...
		showSummary: function (lid, id, ecid, summaryData) {
			...
			this.summaryGID = id;
			this.summaryLID = lid;
			// this.$socket.emit('sendHDP_OU_EARLY', { ecid: ecid, id: id, showtype: "early" }, { lids: this.earlyGids, field: this.field });
			this.changedFTDataList.forEach(element => {
				if (element["lid"] === lid) {
					element["gameList"].forEach(item => {
						if (item["id"] === id) {
