<template>
	<div :style="{ height: loading || changedFTScoreDataList.length === 0 ? '90vh' : 'unset' }">
		...
		<div class="text-center font-bold text-[20px] mt-[20px] text-white" v-if="changedFTScoreDataList.length === 0 && !loading">没有数据</div>
		<div v-for="(lidItem, lidItemIndex) in changedFTScoreDataList" :key="lidItemIndex + 100">
			...
			<div :class="{ detail_show: lidItem['show'], detail_hide: !lidItem['show'] }"
				...
				<div class="ranks">
					...
					<div class="ranks_r text-[14px] font-bold">
						<span></span>
						...
						<div class="score_list" v-for="(subItem, subItemIndex) in scoreItem['score']" :key="subItemIndex">
							<div class="score_item"
								...
								<div class="item-background" v-if="scores.state == 1"
									...
									<span class="text-[14px] font-bold">{{ scores.num }}</span>
								</div>
...
export default defineComponent({
	...
	methods: {
		...
		halfMoreShow: function (lid, gameItemInex, scoreListIndex) {
			this.changedFTScoreDataList.map(item => {
				if (item["lid"] === lid) {
