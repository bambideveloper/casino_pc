...
export default defineComponent({
	...
	sockets: {
		...
		receivedFTChampionMessage(data) {
			let tempReceivedData = [];
			this.loading = false;
			console.log('receivedFTChampionMessage', data);
			...
			let timeArray = tempReceivedData.map(function (item) {
	...
	mounted() {
		this.$socket.emit("sendChampionMainMessage", { lid: this.championLid })
	},
	unmounted() {
		this.$socket.emit("stopFTChampionMessage")
	},
	methods: {
