			...
			<div class="center-item" :class="{ detail_show: item['show'], detail_hide: !item['show'] }"
				...
				<div class="table-text table-list" v-for="(datalist, scoreIndex) in data.scoreList" :key="scoreIndex">
					<div class="table-text-l" v-if="datalist.name">
						...
						<span v-if="datalist.name !== '和'" class="font-bold text-[14px]">{{ datalist.name }}</span>
					</div>
...
export default defineComponent({
	...
	watch: {
		changedFTDataList: function (newDataList) {
		},
	},
	sockets: {
		connect: function () {
		...
		receivedFTTodayMessage: function (data) {
			this.loading = false;
			// console.log('receivedFTTodayMessage', data);
			if (data == null) return;
			var ftDataList = [];
			let lidArray = data.map(function (item) {
				return item["LID"];
			});
		...
		receivedFT_HDP_OU_TODAY: function (data) {
		...
		receivedFT_CORNER_TODAY: function (data) {
			this.loading = false;
			console.log(data);
			for (let i = 0; i < this.changedFTDataList.length; i++) {
	...
	methods: {
		...
		showSummary: function (lid, id, ecid, summaryData) {
			...
			this.summaryGID = id;
			this.summaryLID = lid;
			// this.$socket.emit('sendHDP_OU_TODAY', { ecid: ecid, id: id, showtype: "today" }, { lids: this.todayGids, field: this.field });
			this.changedFTDataList.forEach(element => {
				if (element["lid"] === lid) {
					element["gameList"].forEach(item => {
						if (item["id"] === id) {
						...
						scoreList: [
							{
								...
								nums: [
									...
									},
									{
										...
										colorChangeUp: false,
										colorChangeDOwn: false,
										text: item["MB_Dime"] == "" ? "" : "大 " + item["MB_Dime"].split("O")[1],
								...
								nums: [
									{
										...
										bettingType: "H",
										odd_f_type: "H",
										type: item["S_0_1"] == 0 || item["S_0_1"] == undefined || item["S_0_1"] == "" ? 2 : 1,
										colorChangeUp: false,
										colorChangeDOwn: false,
										text: "0 - 1",
										num: item["S_0_1"] == 0 || item["S_0_1"] == undefined || item["S_0_1"] == "" ? 0 : (Number(item["S_0_1"])).toFixed(2)
									},
									{
										lineType: 6,
										r_type: "2~3",
										bettingType: "H",
										odd_f_type: "H",
										type: item["S_2_3"] == 0 || item["S_2_3"] == undefined || item["S_2_3"] == "" ? 2 : 1,
										colorChangeUp: false,
										colorChangeDOwn: false,
										text: "2 - 3",
										num: item["S_2_3"] == 0 || item["S_2_3"] == undefined || item["S_2_3"] == "" ? 0 : (Number(item["S_2_3"])).toFixed(2)
									},
									{
										lineType: 6,
										r_type: "4~6",
										bettingType: "H",
										odd_f_type: "H",
										type: item["S_4_6"] == 0 || item["S_4_6"] == undefined || item["S_4_6"] == "" ? 2 : 1,
										colorChangeUp: false,
										colorChangeDOwn: false,
										text: "4 - 6",
										num: item["S_4_6"] == 0 || item["S_4_6"] == undefined || item["S_4_6"] == "" ? 0 : (Number(item["S_4_6"])).toFixed(2)
									},
									{
										lineType: 6,
										r_type: "OVER",
										bettingType: "H",
										odd_f_type: "H",
										type: item["S_7UP"] == 0 || item["S_7UP"] == undefined || item["S_7UP"] == "" ? 2 : 1,
										colorChangeUp: false,
										colorChangeDOwn: false,
										text: "7或以上",
										num: item["S_7UP"] == 0 || item["S_7UP"] == undefined || item["S_7UP"] == "" ? 0 : (Number(item["S_7UP"])).toFixed(2)
									}
								]
							}
						],
								...
								nums: [
									{
										...
										bettingType: "H",
										odd_f_type: "H",
										type: item["MBMB"] == 0 || item["MBMB"] == undefined || item["MBMB"] == "" ? 2 : 1,
										colorChangeUp: false,
										colorChangeDOwn: false,
										text: "主 / 主",
										num: item["MBMB"] == 0 || item["MBMB"] == undefined || item["MBMB"] == "" ? 0 : (Number(item["MBMB"])).toFixed(2)
									},
									{
										lineType: 7,
										r_type: "FHN",
										bettingType: "H",
										odd_f_type: "H",
										type: item["MBFT"] == 0 || item["MBFT"] == undefined || item["MBFT"] == "" ? 2 : 1,
										colorChangeUp: false,
										colorChangeDOwn: false,
										text: "主 / 和",
										num: item["MBFT"] == 0 || item["MBFT"] == undefined || item["MBFT"] == "" ? 0 : (Number(item["MBFT"])).toFixed(2)
									},
									{
										lineType: 7,
										r_type: "FHC",
										bettingType: "H",
										odd_f_type: "H",
										type: item["MBTG"] == 0 || item["MBTG"] == undefined || item["MBTG"] == "" ? 2 : 1,
										colorChangeUp: false,
										colorChangeDOwn: false,
										text: "主 / 客",
										num: item["MBTG"] == 0 || item["MBTG"] == undefined || item["MBTG"] == "" ? 0 : (Number(item["MBTG"])).toFixed(2)
									},
									{
										lineType: 7,
										r_type: "FNH",
										bettingType: "H",
										odd_f_type: "H",
										type: item["FTMB"] == 0 || item["FTMB"] == undefined || item["FTMB"] == "" ? 2 : 1,
										colorChangeUp: false,
										colorChangeDOwn: false,
										text: "和 / 主",
										num: item["FTMB"] == 0 || item["FTMB"] == undefined || item["FTMB"] == "" ? 0 : (Number(item["FTMB"])).toFixed(2)
									},
									{
										lineType: 7,
										r_type: "FNN",
										bettingType: "H",
										odd_f_type: "H",
										type: item["FTFT"] == 0 || item["FTFT"] == undefined || item["FTFT"] == "" ? 2 : 1,
										colorChangeUp: false,
										colorChangeDOwn: false,
										text: "和 / 和",
										num: item["FTFT"] == 0 || item["FTFT"] == undefined || item["FTFT"] == "" ? 0 : (Number(item["FTFT"])).toFixed(2)
									}
								]
							},
							{
								nums: [
									{
										...
										bettingType: "H",
										odd_f_type: "H",
										type: item["FTTG"] == 0 || item["FTTG"] == undefined || item["FTTG"] == "" ? 2 : 1,
										colorChangeUp: false,
										colorChangeDOwn: false,
										text: "和 / 客",
										num: item["FTTG"] == 0 || item["FTTG"] == undefined || item["FTTG"] == "" ? 0 : (Number(item["FTTG"])).toFixed(2)
									},
									{
										lineType: 7,
										r_type: "FCH",
										bettingType: "H",
										odd_f_type: "H",
										type: item["TGMB"] == 0 || item["TGMB"] == undefined || item["TGMB"] == "" ? 2 : 1,
										colorChangeUp: false,
										colorChangeDOwn: false,
										text: "客 / 主",
										num: item["TGMB"] == 0 || item["TGMB"] == undefined || item["TGMB"] == "" ? 0 : (Number(item["TGMB"])).toFixed(2)
									},
									{
										lineType: 7,
										r_type: "FHC",
										bettingType: "H",
										odd_f_type: "H",
										type: item["TGFT"] == 0 || item["TGFT"] == undefined || item["TGFT"] == "" ? 2 : 1,
										colorChangeUp: false,
										colorChangeDOwn: false,
										text: "客 / 和",
										num: item["TGFT"] == 0 || item["TGFT"] == undefined || item["TGFT"] == "" ? 0 : (Number(item["TGFT"])).toFixed(2)
									},
									{
										lineType: 7,
										r_type: "FNH",
										bettingType: "H",
										odd_f_type: "H",
										type: item["TGTG"] == 0 || item["TGTG"] == undefined || item["TGTG"] == "" ? 2 : 1,
										colorChangeUp: false,
										colorChangeDOwn: false,
										text: "客 / 客",
										num: item["TGTG"] == 0 || item["TGTG"] == undefined || item["TGTG"] == "" ? 0 : (Number(item["TGTG"])).toFixed(2)
									},
									{
									}
								]
		...
		handleModal1: function (leagueData, gameData, dataList, rateData, scoreIndex) {
			...
			if (this.bettingOrderData["title"].includes("单双") && this.user.FT_EO_Bet == 0) {
				showToast("对不起,本场有下注金额最高:  RMB 0");
				return;
			}
			if (this.bettingOrderData["rate"] == 0 || this.bettingOrderData["rate"] == null) this.openModal = false;
